      ...
      <div className="p-8 max-w-6xl mx-auto space-y-8 animate-fade-in">
        ...
        <div className="flex items-center justify-between border-b border-border pb-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Full audit trail of all changes</p>
          </div>
        </div>

        {/* FILTERS */}
        <div className="flex flex-col md:flex-row gap-4 items-center bg-muted p-4 rounded-2xl border border-border">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-20 border border-dashed border-border rounded-2xl bg-muted">
            <History size={48} className="text-muted-foreground mb-4" />
            <h3 className="text-xl font-bold mb-1">No logs found</h3>
            <p className="text-muted-foreground text-sm">Try adjusting your filters</p>
          </div>
        ) : (
          <div className="aurelia-card border border-border overflow-hidden shadow-2xl">
            <table className="w-full text-left text-sm">
              <thead className="bg-muted text-[10px] text-muted-foreground font-bold uppercase tracking-widest">
                <tr>
                  ...
                  <tr key={log.id} className="hover:bg-muted/40 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-xs uppercase tracking-tighter text-muted-foreground">{log.changed_by}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 min-w-[240px]">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="px-1.5 py-0.5 bg-muted rounded text-[9px] font-black uppercase tracking-widest text-muted-foreground border border-border">
                          {log.table_name}
                        </span>
                        <span className="text-[10px] text-muted-foreground font-mono">{log.record_id}</span>
                      </div>
                      <p className="text-foreground font-medium leading-snug">{log.description}</p>
                    </td>
                    <td className="px-6 py-4">
                      {log.field_name && (
                        <div className="flex items-center gap-2 text-[10px] font-mono">
                          <div className="max-w-[100px] truncate text-red-700/70 line-through bg-red-600/5 px-1.5 py-0.5 rounded">
                            {log.old_value}
                          </div>
                          <ChevronRight size={10} className="text-muted-foreground" />
                          <div className="max-w-[100px] truncate text-green-700 bg-green-600/5 px-1.5 py-0.5 rounded border border-green-600/10">
                            {log.new_value}
                          </div>
                        </div>
                      )}
                    ...
                    <td className="px-6 py-4 text-right">
                      {isReversible(log) && (
                        <button 
                          ...
                          className="px-3 py-1.5 bg-muted border border-border rounded-lg text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-gold hover:border-gold/30 hover:bg-gold/5 transition-all flex items-center gap-2 ml-auto"
                        >
                          <RotateCcw size={12} />
                          Undo
                        </button>
